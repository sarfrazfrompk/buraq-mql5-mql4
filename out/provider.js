...
let err_codes = {};
try { err_codes = require('../mql-data/error-codes.json'); } catch (e) { err_codes = {}; }
const obj_items = require('../mql-data/items.json');
const colorW = require('../mql-data/color.json')
...
function Hover_log() {
    return {
        provideHover(document, position) {
            ...
            if (!ext.obj_hover || !(word in ext.obj_hover)) return undefined;
...
function DefinitionProvider() {
    return {
        provideDefinition(document, position) {
            ...
            const f = link.match(/.+(?=#)/g) !== null ? link.match(/(?<=#)(?:\d+,\d+)$/gm)[0] : '0,0';
            const parts = f.split(',');
            const uri = vscode.Uri.parse(fileLink);
            const pos = new vscode.Position(+parts[0] <= 0 ? 0 : +parts[0] - 1, +parts[1] <= 0 ? 0 : +parts[1] - 1);
...
function ItemProvider() {
    return {
        provideCompletionItems(document, position) {
            const loclang = language === 'zh-tw' ? 'zh-cn' : language,
                path = vscode.Uri.file(pathModule.join(__dirname, '../', 'mql-images', 'mql-icon.png')),
                range = document.getWordRangeAtPosition(position),
